...
open class SQLServer @JvmOverloads constructor(
    ...
    override val driverName = "com.microsoft.sqlserver.jdbc.SQLServerDriver"
    ...
    override fun getConnection(jdbcUrl: String): Connection {
        ...
        return SQLServerDriver().connect(jdbcUrl, properties)
    }

    override fun getJDBCUrl(): String = "jdbc:sqlserver://$host:$port;databaseName=$schema"

    override fun formatSelect(select: Select): String {
        return buildString {
            val topString = if(select.limit != -1) " TOP ${select.limit}" else ""
            if(select.isDistinct) append("SELECT DISTINCT$topString ") else append("SELECT$topString ")
